...
interface RealtimeTranslationSessionProps {
  userId: string;
}

export default function RealtimeTranslationSession({ userId }: RealtimeTranslationSessionProps) {
  ...
  const { isRecording, isProcessing, startRecording, stopRecording } = useRealtimeTranslation({
    ...
    onAccumulatedUpdate: (transcription) => {
      ...
      setPartialTranscription(null); // Clear partial when we have accumulated
    },
    onFinalTranscription: (transcription) => {
      setSavedTranscriptions(prev => [...prev, transcription]);
      setAccumulatedTranscription(null); // Clear accumulated when saved
    },
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          const newTranscription = payload.new as FinalTranscription;
  ...
  return (
    <div className="container mx-auto p-6 max-w-6xl">
      <Card>
        <CardHeader>
          <CardTitle>🎙️ Real-Time Lecture Translation (Streaming)</CardTitle>
          <CardDescription>
            Record lectures with live streaming transcription and translation
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          {/* Accumulated Transcription (Growing as you speak) */}
          {sessionId && isRecording && (accumulatedTranscription || partialTranscription) && (
            <div className="space-y-2">
                  ...
                  <div className="space-y-3">
                    <div>
                      <p className="text-xs text-muted-foreground mb-1">
                        Original (English) - {accumulatedTranscription ? 'Recording...' : 'Listening...'}
                      </p>
                      <p className="text-base leading-relaxed">
                        {accumulatedTranscription?.originalText || ''}
                        {partialTranscription && !accumulatedTranscription && (
                          <span className="italic text-muted-foreground">{partialTranscription.originalText}</span>
                        )}
                        {partialTranscription && accumulatedTranscription && (
                          <span className="italic text-muted-foreground"> {partialTranscription.originalText}</span>
                        )}
                    ...
                    <div className="border-t pt-3">
                      <p className="text-xs text-muted-foreground mb-1">
                        Translation ({LANGUAGES.find(l => l.code === targetLanguage)?.name}) - {accumulatedTranscription ? 'Translating...' : 'Waiting...'}
                      </p>
                      <p className="text-base font-medium leading-relaxed">
                        {accumulatedTranscription?.translatedText || ''}
                        {partialTranscription && !accumulatedTranscription && (
                          <span className="italic text-muted-foreground">{partialTranscription.translatedText}</span>
                        )}
                        {partialTranscription && accumulatedTranscription && (
                          <span className="italic text-muted-foreground"> {partialTranscription.translatedText}</span>
                        )}
                    ...
                    {accumulatedTranscription && (
                      <div className="border-t pt-2">
                        <p className="text-xs text-muted-foreground">
                          Started: {new Date(accumulatedTranscription.startedAt).toLocaleTimeString()}
                        </p>
                      </div>
                    )}
                  </div>
          ...
          {sessionId && savedTranscriptions.length > 0 && (
            <div className="space-y-4">
              <h3 className="font-semibold text-lg">Saved Transcriptions ({savedTranscriptions.length})</h3>
